...
# TikTok Crypto Video Builder (v4 - robust + absolute paths + bugfix)
# Requires: ffmpeg 4+, optional espeak-ng
# Usage: ./make_tiktok_v4.sh
...
if [ ! -f "audio/voiceover.mp3" ]; then
  if has_cmd espeak-ng && [ -f "voiceover.txt" ]; then
    ...
    # More stable: write a temporary WAV first, then encode to mp3
    espeak-ng -s 155 -v en-US+m3 -f voiceover.txt -w build/voiceover_tmp.wav
    ffmpeg -nostdin -y -i build/voiceover_tmp.wav -ac 2 -ar 48000 -b:a 192k audio/voiceover.mp3
    rm -f build/voiceover_tmp.wav
  else
...
if [ -f "audio/music.mp3" ]; then
  ...
  ffmpeg -nostdin -y -i audio/music.mp3 -af loudnorm=I=-22:TP=-1.5:LRA=11 build/music_norm.mp3
...
while IFS=, read -r FNAME START DURATION CUE; do
  lineno=$((lineno+1))
  # Skip header or blank/comment lines
  ...
  BASENAME="${FNAME%%.png}"
  echo "  line $lineno -> scene=$FNAME start=${START}s duration=${DURATION}s cue=$CUE"

  ffmpeg -nostdin -y -loop 1 -t "$DURATION" -i "scenes/$FNAME" \
    ...
    -r ${FPS} -pix_fmt yuv420p -an "build/segs/${BASENAME}.mp4"

  echo "file '$PWD_ABS/build/segs/${BASENAME}.mp4'" >> "$CONCAT_FILE"
done < build/storyboard.norm.csv

echo "Concatenating segments..."
ffmpeg -nostdin -y -f concat -safe 0 -i "$CONCAT_FILE" -c copy "build/video_nocaptions.mp4"

# Burn captions (ASS) if available
if [ -n "$CAPTIONS" ]; then
  echo "Burning captions..."
  ffmpeg -nostdin -y -i "build/video_nocaptions.mp4" -vf "ass=${CAPTIONS}" -c:a copy "build/video_captions.mp4"
...
if [ -f "audio/voiceover.mp3" ] && [ -n "$MUSIC" ]; then
  ...
  ffmpeg -nostdin -y -i "$VIDEO" -i audio/voiceover.mp3 -i "$MUSIC" \
  ...
  ffmpeg -nostdin -y -i "$VIDEO" -i audio/voiceover.mp3 -map 0:v -map 1:a -c:v libx264 -pix_fmt yuv420p -r ${FPS} -c:a aac -b:a 192k -shortest "$OUT"
elif [ -n "$MUSIC" ]; then
  echo "Muxing music only..."
  ffmpeg -nostdin -y -i "$VIDEO" -i "$MUSIC" -map 0:v -map 1:a -c:v libx264 -pix_fmt yuv420p -r ${FPS} -c:a aac -b:a 192k -shortest "$OUT"
else
  echo "No audio sources found. Exporting silent video..."
  ffmpeg -nostdin -y -i "$VIDEO" -c:v libx264 -pix_fmt yuv420p -r ${FPS} -an "$OUT"
fi
