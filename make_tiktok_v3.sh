...
# TikTok Crypto Video Builder (v3 - absolute concat paths)
# Requires: ffmpeg 4+, optional espeak-ng
# Usage: ./make_tiktok_v3.sh
...
if [ ! -f "audio/voiceover.mp3" ]; then
  if command -v espeak-ng >/dev/null 2>&1 && [ -f "voiceover.txt" ]; then
...
while IFS=, read -r FNAME START DURATION CUE; do
  ...
  # Write ABSOLUTE path so concat demuxer doesn't get confused
